# Deployment evidence — 20260808-080321Z
...
| Metric | Value | Source |
|---|---|---|
| NF profiles registered in NRF |  | MongoDB `NfProfile` |
| Subscribers provisioned |  | MongoDB `amData` |
| **UEs RM-REGISTERED (ground truth)** | **0** | `nr-cli … -e status` |
| UEs logging "Initial Registration is successful" |  | UE log |
| Prometheus targets healthy | 0
0 | Prometheus API |
| AMF `ue_connectivity{3GPP}` | n/a | AMF exporter (unreliable — see below) |
| AMF `ue_cm_gmm_state_count{cm-connected}` | n/a | AMF exporter (unreliable — see below) |
