# Deployment evidence — 20260916-055327Z
...
| Metric | Value | Source |
|---|---|---|
| NF profiles registered in NRF | 18 | MongoDB `NfProfile` |
| Subscribers provisioned | 20 | MongoDB `amData` |
| **UEs RM-REGISTERED (ground truth)** | **20** | `nr-cli … -e status` |
| UEs logging "Initial Registration is successful" | 20 | UE log |
| Prometheus targets healthy | 10 | Prometheus API |
| AMF `ue_connectivity{3GPP}` | -10 | AMF exporter (unreliable — see below) |
| AMF `ue_cm_gmm_state_count{cm-connected}` | 20 | AMF exporter (unreliable — see below) |
