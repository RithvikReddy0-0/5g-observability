...
lbl=$(docker image inspect o5gs/ueransim:v3.3.0-udpbuf-mono --format '{{index .Config.Labels "io.5g-observability.patches"}}' 2>/dev/null)
p_ok=1; for p in ueransim-udp-socket-buffers ueransim-monotonic-clock; do
  [ -f "deployments/open5gs/images/patches/$p.patch" ] && echo " $lbl " | grep -q " $p " || p_ok=0
done
[ "$p_ok" = 1 ] \
  && ok "UERANSIM image built from the pin with its recorded local patches ($lbl; ADR-012, ADR-014)" \
  || no "UERANSIM image or its patch records missing (label: '${lbl:-none}')"
tuns=$(for u in o5gs-upf-embb o5gs-upf-urllc o5gs-upf-mmtc; do docker exec "$u" sh -c 'ls /sys/class/net | grep -c ogstun' 2>/dev/null; done | paste -sd+ | bc 2>/dev/null)
[ "${tuns:-0}" -ge 3 ] && ok "user plane runs with no kernel module (userspace UPF over TUN, one per slice)" || no "UPF TUN devices missing"
...
want="o5gs-mongodb o5gs-nrf o5gs-scp o5gs-ausf o5gs-udm o5gs-udr o5gs-pcf o5gs-bsf o5gs-nssf o5gs-amf o5gs-smf o5gs-upf-embb o5gs-upf-urllc o5gs-upf-mmtc o5gs-gnb-embb o5gs-gnb-urllc o5gs-gnb-mmtc o5gs-ue o5gs-dn-embb o5gs-dn-urllc o5gs-dn-mmtc o5gs-prometheus o5gs-grafana"
...
gnb_ok=0; for g in o5gs-gnb-embb o5gs-gnb-urllc o5gs-gnb-mmtc; do
...
[ "$gnb_ok" -eq 3 ] && ok "all three slice gNBs completed NG Setup with the AMF" || no "only $gnb_ok/3 gNBs completed NG Setup"
peers=$(metric 10.53.0.4:9090 | awk '/^pfcp_peers_active/ {print $2}')
[ "${peers:-0}" -ge 3 ] && ok "SMF associated over N4 with all three slice UPFs ($peers PFCP peers)" || no "SMF PFCP peers: ${peers:-none}"
...
[ "${subs%% *}" = "100" ] && echo "$subs" | grep -q '"1/010203":20' && echo "$subs" | grep -q '"2/112233":10' \
  && echo "$subs" | grep -q '"3/334455":70' \
  && ok "100 subscribers: eMBB 20, URLLC 10, mMTC 70 by home slice ($subs)" || no "subscribers: ${subs:-unreadable}"
...
[ "${reg:-0}" -ge 100 ] && ok "UE registration: $reg/100 UEs registered" || no "UE registration: ${reg:-0}/100"
...
na=$(echo "$live" | grep -c '^10\.45\.'); nb=$(echo "$live" | grep -c '^10\.46\.'); nc=$(echo "$live" | grep -c '^10\.47\.')
[ "$na" -ge 20 ] && [ "$nb" -ge 10 ] && [ "$nc" -ge 70 ] \
  && ok "PDU session established, UE addressed from its slice's pool: eMBB $na/20, URLLC $nb/10, mMTC $nc/70" \
  || no "PDU sessions with a live address: eMBB $na/20, URLLC $nb/10, mMTC $nc/70"
if bash scripts/open5gs/verify_user_plane.sh > /tmp/acc-up.txt 2>&1; then
  ok "end-to-end user plane: ping and iperf3 through the UPF on all three slices, internet egress ($(grep -oE 'PASS=[0-9]+ +FAIL=[0-9]+' /tmp/acc-up.txt))"
...
s_mmtc=$(metric 10.53.0.9:9090 | awk '/^fivegs_upffunction_upf_sessionnbr/ {print $2}')
[ "${s_embb:-0}" -gt 0 ] && [ "${s_urllc:-0}" -gt 0 ] && [ "${s_mmtc:-0}" -gt 0 ] \
  && ok "each slice's traffic is carried by its own UPF (sessions: eMBB UPF $s_embb, URLLC UPF $s_urllc, mMTC UPF $s_mmtc — presence, ADR-011)" \
  || no "per-slice UPF sessions: eMBB ${s_embb:-none}, URLLC ${s_urllc:-none}, mMTC ${s_mmtc:-none}"
